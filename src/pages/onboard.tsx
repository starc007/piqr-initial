...
const Onboard = (props: Props) => {
  ...
  const checkIfUsernameTaken = debounce(
  ...
  const handleOnboard: SubmitHandler<FormData> = async (data) => {
    setLoading(true);
    await updateProfile({ ...data, category: skills }).then(() =>
      Router.push("/explore")
    );
    setLoading(false);
  };
  ...
  useEffect(() => {
    if (user?.profile?.user?.firstTime === false) {
      Router.push("/explore");
    }
  }, [user]);
        ...
        <form onSubmit={handleSubmit(handleOnboard)}>
          <div className="mt-6">
            ...
            <div className="mt-2.5 relative text-gray-400 focus-within:text-gray-600">
              ...
              <Input
                {...register("username", {
                  pattern: {
                    ...
                    message:
                      "Invalid Username ( Length must be greater than 3 and less than 15 , should not contain space )",
                  },
                })}
                min={3}
                max={15}
              ...
              {errors.username?.message && (
                <span className="my-2 text-red-500 font-medium text-sm">
                  {errors?.username.message}
                </span>
              )}
          ...
          <div className="mt-3">
            ...
            <div className="mt-2.5 relative text-gray-400 focus-within:text-gray-600">
              <TagsInput
                className="!p-4 h-12"
                placeholder="Eg. Frontend, Backend, Fullstack, etc."
                tags={skills}
                setTags={setSkills}
